...
import type { GateResultType } from 'shared-types';

const SLACK_WEBHOOK = process.env.SLACK_WEBHOOK_URL;

export async function alertGateFailure(
  briefId: string,
  date: string,
  failedGates: GateResultType[]
): Promise<void> {
  if (!SLACK_WEBHOOK) {
...
export async function alertPublishSuccess(date: string, briefId: string): Promise<void> {
  if (!SLACK_WEBHOOK) {
...
export async function alertIngestionFailures(
  date: string,
