import type { BriefOutputType, GateResultType } from 'shared-types';
...
export function runGate1ProhibitedLanguage(brief: BriefOutputType): GateResultType {
  const fullText = [
...
export function runGate2SourceCompleteness(brief: BriefOutputType): GateResultType {
  const missing = brief.expert.signals.filter(
    s => !s.source_url || s.source_url.trim() === ''
  );
...
export function runGate3DisclaimerPresence(brief: BriefOutputType): GateResultType {
...
export function runAllGates(brief: BriefOutputType): {
  allPassed: boolean;
  results: GateResultType[];
} {
  const results = [
