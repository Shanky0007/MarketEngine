...
import { runGate1ProhibitedLanguage } from '../gates.js';

const FALLBACK_CONTEXT =
  'Insufficient historical data to establish a pattern. Past patterns do not predict future outcomes.';
...
async function synthesiseOneAlert(
  anomaly: AnomalyEventType,
...
30-day score history: ${JSON.stringify(historyPayload)}`
    ...
    console.error(`[Alert Synthesiser] OpenAI call failed for ${anomaly.sector}/${anomaly.anomaly_type}:`, (err as Error).message);
    return FALLBACK_CONTEXT;
  }
  ...
  try {
    const parsed = JSON.parse(raw) as { historical_context?: string };
  ...
  if (!historicalContext) return FALLBACK_CONTEXT;
  ...
  const gate1 = runGate1ProhibitedLanguage(mockBrief);
  if (!gate1.passed) {
    console.error(
      `[Alert Synthesiser] Gate 1 failed for ${anomaly.sector}/${anomaly.anomaly_type}: ${gate1.details}`
    );
    return FALLBACK_CONTEXT;
  }
...
export async function synthesiseAlerts(
  anomalies: AnomalyEventType[],
  historicalScores: SectorScoreType[]
): Promise<AnomalyEventType[]> {
  const enriched: AnomalyEventType[] = [];
