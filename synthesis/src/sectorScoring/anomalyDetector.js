...
const ANOMALY_DISCLAIMER = 'This alert describes historical data patterns. It is not financial advice. ' +
...
function sectorHistory(sector, historical) {
    return historical
...
function checkConsecutiveFIISell(currentSignals, historical) {
    const anomalies = [];
    const sectors = ['IT', 'BANKING', 'PHARMA', 'AUTO', 'FMCG', 'ENERGY', 'METALS'];
    for (const sector of sectors) {
        const current = currentSignals[sector];
        if (current.fii_net_cr === null || current.fii_net_cr >= 0)
            continue;
        ...
        if (!allNegative)
            continue;
        const totalOutflow = hist.reduce((sum, s) => sum + (s.signal_breakdown.fii_net_cr ?? 0), 0) +
            current.fii_net_cr;
...
function checkPCRThresholdBreach(currentSignals) {
    const anomalies = [];
    const sectors = ['IT', 'BANKING', 'PHARMA', 'AUTO', 'FMCG', 'ENERGY', 'METALS'];
    for (const sector of sectors) {
        ...
        let threshold = null;
        if (pcr < 0.65)
            threshold = 'below 0.65 (low put activity)';
        if (pcr > 1.6)
            threshold = 'above 1.6 (extreme put buying)';
        if (!threshold)
            continue;
        anomalies.push({
            sector,
...
function checkScoreReversal(currentScores, historical) {
    const anomalies = [];
...
function checkLargeBlockDeal(currentSignals) {
    const anomalies = [];
    const sectors = ['IT', 'BANKING', 'PHARMA', 'AUTO', 'FMCG', 'ENERGY', 'METALS'];
    for (const sector of sectors) {
        const net = currentSignals[sector].block_deal_net_cr;
        if (net === null || Math.abs(net) <= 500)
            continue;
...
function checkDIIAbsorptionFailure(currentSignals) {
    const anomalies = [];
    const sectors = ['IT', 'BANKING', 'PHARMA', 'AUTO', 'FMCG', 'ENERGY', 'METALS'];
    for (const sector of sectors) {
...
function checkSectorDivergence(currentScores) {
    const anomalies = [];
    if (currentScores.length === 0)
        return anomalies;
    const marketScore = Math.round(currentScores.reduce((sum, s) => sum + s.score, 0) / currentScores.length);
    if (marketScore >= 40)
        return anomalies;
...
// ─── Main export ──────────────────────────────────────────────────────────────
function detectAnomalies(currentScores, historicalScores, currentSignals) {
    return [
