...
exports.mapScripToSector = mapScripToSector;
exports.mapNameToSector = mapNameToSector;
exports.mapCompany = mapCompany;
exports.aggregateBlockDealsBySector = aggregateBlockDealsBySector;
const sectorMap_json_1 = __importDefault(require("./data/sectorMap.json"));
...
// ─── Public API ───────────────────────────────────────────────────────────────
function mapScripToSector(scripCode) {
    return scripToSector[scripCode.trim()] ?? null;
}
function mapNameToSector(name) {
...
function mapCompany(scripCode, name) {
    return mapScripToSector(scripCode) ?? mapNameToSector(name);
}
function aggregateBlockDealsBySector(deals) {
    const result = {};
    for (const sector of SECTORS) {
