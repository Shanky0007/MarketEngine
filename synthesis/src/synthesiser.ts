...
import { AggregatedSignals, BriefOutput } from 'shared-types';
...
type SynthesisResult = {
  brief: BriefOutputType | null;
  audit: AuditRecordType;
  gatesPassed: boolean;
};

export async function synthesiseBrief(
  aggregated: AggregatedSignalsType
  ...
  // Parse and validate
  let brief: BriefOutputType | null = null;
  try {
    const parsed = JSON.parse(rawOutput);
    parsed.brief_id = briefId;
    brief = BriefOutput.parse(parsed);
  ...
  // Run gates
  const gateResults = brief ? runAllGates(brief) : { allPassed: false, results: [] };

  // Build audit record
  const audit: AuditRecordType = {
    brief_id: briefId,
